  ...
  <div class="flex items-center">
    ...
    <div class="flex-grow mx-2 flex">
      ...
      >
        <input
          type="radio"
          :id="'invocation_' + option"
          :value="option"
          v-model="selectedOption"
          @change="updateInvocation"
          name="invocation"
          class="hidden"
        />
        <label
          :for="'invocation_' + option"
...
export default {
  ...
  methods: {
    updateInvocation() {
      this.$emit('update-filter', { invocation: this.selectedOption === 'Yes' })
    },
    resetInvocation() {
      this.selectedOption = ''
      this.updateInvocation()
    }
  }
}
</script>
