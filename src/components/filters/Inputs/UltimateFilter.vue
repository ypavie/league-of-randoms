<template>
  <div class="flex items-center">
    <label
      for="ultimate"
      class="w-32 dark:text-gray-400 text-gray-700 font-bold text-right flex-shrink-0"
    >
      Ultimate:
    </label>
    <div class="flex-grow mx-2 flex">
      <div
        v-for="(option, index) in options"
        :key="option"
        :class="{
          'grow mr-2': index < options.length - 1,
          grow: index === options.length - 1
        }"
      >
        <input
          type="radio"
          :id="option"
          :value="option"
          v-model="selectedOption"
          @change="updateUltimate"
          name="ultimate"
          class="hidden"
        />
        <label
          :for="option"
          :class="[
            'cursor-pointer block px-4 py-2 border rounded-md shadow-sm flex items-center justify-center',
            selectedOption === option
              ? 'bg-gray-300 text-gray-700'
              : 'bg-white text-gray-700 dark:bg-gray-700 dark:text-white dark:text-gray-400'
          ]"
          @click="updateUltimate(option)"
        >
          {{ option }}
        </label>
      </div>
    </div>

    <button
      @click="resetUltimate"
      class="p-1 text-red-500 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 focus:outline-none"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        class="h-4 w-4"
        viewBox="0 0 20 20"
        fill="currentColor"
      >
        <path
          fill-rule="evenodd"
          d="M5.293 5.293a1 1 0 0 1 1.414 1.414L10 11.414l3.293-3.293a1 1 0 1 1 1.414 1.414L11.414 12l3.293 3.293a1 1 0 1 1-1.414 1.414L10 13.414l-3.293 3.293a1 1 0 1 1-1.414-1.414L8.586 12 5.293 8.707a1 1 0 0 1 0-1.414z"
          clip-rule="evenodd"
        />
      </svg>
    </button>
  </div>
</template>

<script>
export default {
  name: 'UltimateFilter',
  data() {
    return {
      options: ['Classic', 'Semi', 'Global'],
      selectedOption: ''
    }
  },
  methods: {
    updateUltimate(option) {
      if (this.selectedOption === option) {
        this.selectedOption = ''
      } else {
        this.selectedOption = option
      }
      let value
      if (this.selectedOption === this.options[0]) {
        value = 'none'
      } else if (this.selectedOption === this.options[1]) {
        value = 'semi'
      } else if (this.selectedOption === this.options[2]) {
        value = 'global'
      } else {
        value = ''
      }
      this.$emit('update-filter', { ultimate: value })
    },
    resetUltimate() {
      this.selectedOption = ''
      this.updateUltimate('')
    }
  }
}
</script>
